...
try:
    ...
    if verschil_dagen > 0:
        ...
        wa_bericht = f"⏳ *De teller tikt door!*\n\nNog precies {verschil_dagen} dagen wachten op *{naam}*. Kijken jullie er ook al naar uit?\n\nBekijk de live aftelklok hier:\n{event_url}"
    elif verschil_dagen == 0:
        countdown_tekst = f"{vandaag_leesbaar}: Het is zover!"
        wa_bericht = f"🎉 *Het wachten is voorbij!*\n\nVandaag is eindelijk de dag van *{naam}*!\n\nBekijk de details hier:\n{event_url}"
    else:
        countdown_tekst = f"{vandaag_leesbaar}: Evenement is afgelopen"
        wa_bericht = f"Terugblik: *{naam}* is inmiddels alweer achter de rug.\n\nKijk op de site voor aankomende evenementen:\nhttps://hoelangnogtot.nl/"

except ValueError:
    countdown_tekst = "Fout bij inladen datum"
    event_leesbaar = datum_str
    wa_bericht = f"Praat mee over *{naam}*!\n\nLink: {event_url}"

# Sla het WhatsApp-bericht op in een tekstbestand
with open("whatsapp_bericht.txt", "w", encoding="utf-8") as f:
    f.write(wa_bericht)
...
with sync_playwright() as p:
    ...
    card_element = page.locator("#wa-card")
    card_element.screenshot(path="whatsapp_kaart.png", omit_background=True)
    
    browser.close()
    print("WhatsApp afbeelding en tekst succesvol gegenereerd.")
